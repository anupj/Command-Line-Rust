...
pub fn run(config: Config) -> MyResult<()> {
    ...
    // closure to handle the printing of
    // output with or without filenames
...
// Find the lines that:
// 1) contains the pattern if `invert_match` is false
// 2) does not contain the pattern if `invert_match` is true
                    ...
                    if metadata.is_dir() {
                        if recursive {
                            for entry in WalkDir::new(path)
                                .into_iter()
                                // `flatten` will take the Ok or Some variants
                                // for `Result` and `Option` types and
                                // ignore the `Err` and `None` variants
                                .flatten()
