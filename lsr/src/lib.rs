...
fn format_output(paths: &[PathBuf]) -> MyResult<String> {
    ...
    for path in paths {
        ...
        let user = get_user_by_uid(uid)
            ...
            // if we can't get the username then return the uid as name
            .unwrap_or_else(|| uid.to_string());

        // Get the group id
        let gid = metadata.gid();
        // Either get the group name or group id as `String`
...
mod test {
    ...
    // this helper function will inspect the long output
    // for any one directory entry
    fn long_match(
        line: &str,
        expected_name: &str,
        expected_perms: &str,
        ...
        // use `Iterator::last` rather than try to use
        // positive offset because the modification date
        // column has whitespace
