...
pub fn get_args() -> MyResult<Config> {
    ...
    // If -y|--year is present, set the year to
    // the current year and the month to None.
...
fn parse_year(year: &str) -> MyResult<i32> {
    // Rust infers the type for `parse_int` from the
    // function's return type, `i32`
    // Use `Option::and_then` to handle and `Ok`
    // result from `parse_int`
    parse_int(year).and_then(|num| {
        // check that the parsed number `num`
        // is in the range 1-9999 inclusive
        // of the upper bound
...
fn parse_month(month: &str) -> MyResult<u32> {
    match parse_int(month) {
        ...
        _ => {
            // It means you are parsing a month name
            ...
            let matches: Vec<_> = MONTH_NAMES
                ...
                .enumerate()
                .filter_map(|(i, name)| {
                    // See if the given value is the start of
                    // a month name
                    if name.to_lowercase().starts_with(lower) {
                        // return zero based index position
                        // corrected to one-based counting
                        Some(i + 1)
                    } else {
                        None
                    }
...
fn last_day_in_month(year: i32, month: u32) -> NaiveDate {
    ...
    // if this is Dec, then advance the year by one
...
fn format_month(
    ...
    // Construct a `NaiveDate` for the start
    // of the given month
    let first = NaiveDate::from_ymd_opt(year, month, 1).unwrap();
    // initialise a Vec with a buffer of string with 2 spaces
    // from Sunday until the start of the month
    let mut days: Vec<String> = (1..first.weekday().number_from_sunday())
        .into_iter()
        .map(|_| "  ".to_string()) // two spaces
        .collect();

    // closure to determine if a
    // given day of the month is today
    ...
    // Get the month name from the const array
